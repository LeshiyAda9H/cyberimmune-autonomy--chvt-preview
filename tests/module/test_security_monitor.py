...
from src.config import CARGO_BAY_QUEUE_NAME, COMMUNICATION_GATEWAY_QUEUE_NAME, \
    CONTROL_SYSTEM_QUEUE_NAME, SAFETY_BLOCK_QUEUE_NAME, SERVOS_QUEUE_NAME
...
def test_security_policies(security_monitor):
    ...
    assert security_monitor._check_event(allowed_event) == True

    # Запрещённое взаимодействие
    forbidden_event = Event(
        source=CONTROL_SYSTEM_QUEUE_NAME,
        destination=SERVOS_QUEUE_NAME,
        operation="set_speed"
    )
    assert security_monitor._check_event(forbidden_event) == False